...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    NSLog(@"Sort direction: %c", [settings
                                  boolForKey:kSortDirectionAscending]);
    NSLog(@"Sort field: %@", [settings objectForKey:kSortField]);
    return YES;
}
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    ...
    if (![_persistentStoreCoordinator addPersistentStoreWithType:NSSQLiteStoreType configuration:nil URL:storeURL options:nil error:&error]) {
        ...
        if (success)
        {
            NSLog(@"Core Data Model converted successfully");
            return _persistentStoreCoordinator;
        }
        else
        {
            NSLog(@"Error Converting Core Data: %@, %@", error, error.userInfo);
            abort();
        }
